...
const UI1: React.FC<UI1Props> = ({ partij, forgiveness, onBack }) => {
    ...
    } = useMusicPlayer({ partij, forgiveness, videoRef });
            ...
            <AnimatePresence>
                {!isPlaying && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="absolute top-6 left-1/2 -translate-x-1/2 p-4 bg-black/50 backdrop-blur-lg rounded-2xl border border-white/20 z-20"
                    >
